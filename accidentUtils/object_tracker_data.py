...
from core_similarity_data import *
...
def accidentDetectionData(video_path,output,saved_model_loaded):
    ...
    set_id={}
    list_id=[]
    drive_dir="/content/drive/MyDrive/yolov4-deepsort/detections/"
       ...
       # print('Frame #: ', frame_num)
        ...
        for track in tracker.tracks:
            ...
            if info:
                ...
                img_name = class_name + '.' + str(track.track_id) + '.' + str(frame_num) + '.png'
                # save image
                if track.track_id in set_id:
                  path =os.path.join(drive_dir, str(track.track_id),"*")
                  list_of_files = glob.glob(path)
                  latest_file = max(list_of_files, key=os.path.getctime)
                  img1 = cv2.imread(latest_file,0)
                  ...
                  if len(set_id) ==0:
                    ...
                    sim_list=[]
                    for i in set_id:
                      path =os.path.join(drive_dir, str(i),"*")
                      list_of_files = glob.glob(path)
                      latest_file = max(list_of_files, key=os.path.getctime)
                      img1 = cv2.imread(latest_file,0)
                      sim_list.append([similarity(img1,cropped_img),str(i)])
                    max_similarity=max([sublist for sublist in sim_list])
                    if max_similarity[0]>20:
                      path =os.path.join(drive_dir,max_similarity[1] )
                      img_name = class_name + '.' + str(max_similarity[1]) + '.' + str(frame_num) + '.png'
                      list_id.pop()
                    else:
                      path =os.path.join(drive_dir, str(track.track_id))
                      os.mkdir(path)                      
            ...
            for i in set_id:
              ...
              if length % 30 == 0:
                sorted_files = sorted(os.listdir(path), key=lambda x: int(x.split('.')[2]))[-30:]
                images=[]
                for s in sorted_files:
                  images.append(cv2.imread(os.path.join(path, s )))
                (h,w)=(200,250)
                resized_images=[]
                for k in images:
                  resized_images.append(cv2.resize(k, (w,h), interpolation = cv2.INTER_AREA))
                video_name =str(i)  +'.' + str(length) +'.avi'
                out_s = cv2.VideoWriter(video_name,cv2.VideoWriter_fourcc(*'DIVX'), 15, (w,h))
                #list_video=[]
                for n in resized_images:
                  out_s.write(n)
